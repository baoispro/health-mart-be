...
import { DiscountType, OrderPromotion } from '../entities/order_promotions.entity';
...
export class OrderPromotionsService {
  ...
  async findByOrderId(orderId: number): Promise<OrderPromotion[]> {
    ...
    if (!orderExists) {
      throw new RpcException(new NotFoundException(`Đơn hàng với ID ${orderId} không tồn tại`));
    }
    ...
    if (!promotions || promotions.length === 0) {
      throw new RpcException(new NotFoundException(`Đơn hàng với ID ${orderId} không có khuyến mãi`));
    }
  
    return promotions;
  }

  async createPromotion(dto: CreateOrderPromotionRequest): Promise<OrderPromotion> {
    const order = await this.orderRepository.findOne({ where: { id: dto.order_id } });
    if (!order) {
      throw new RpcException(new NotFoundException(`Không tìm thấy đơn hàng với ID ${dto.order_id}`));
    }
    if (!dto.promoCode || dto.promoCode.trim() === '') {
      throw new RpcException(new BadRequestException('Mã khuyến mãi là bắt buộc và không được để trống'));
    }
  
    if (dto.discountType === DiscountType.FIXED) {
      if (typeof dto.discountValue !== 'number' || dto.discountValue <= 5000) {
        throw new RpcException(
          new BadRequestException('Với discountType FIXED, discountValue phải là số tiền lớn hơn 5000')
        );
      }
    } else if (dto.discountType === DiscountType.PERCENTAGE) {
      if (typeof dto.discountValue !== 'number' || dto.discountValue < 0 || dto.discountValue > 100) {
        throw new RpcException(
          new BadRequestException('Với discountType PERCENTAGE, discountValue phải là số nằm trong khoảng từ 0 đến 100')
        );
      }
    } else if (dto.discountType === DiscountType.NONE) {
      if (dto.discountValue !== undefined && dto.discountValue !== null && dto.discountValue !== 0) {
        throw new RpcException(
          new BadRequestException('Với discountType NONE, discountValue không được nhập hoặc phải bằng 0')
        );
      }
    }
    ...
    if (existingPromotion) {
      throw new RpcException(
        new BadRequestException(`Mã khuyến mãi ${dto.promoCode} đã tồn tại cho đơn hàng với ID ${dto.order_id}`)
      );
    }
  ...
  async updatePromotion(id: number, updateDto: UpdateOrderPromotionRequest): Promise<OrderPromotion> {
    const promotion = await this.orderPromotionRepository.findOneBy({ id });
    if (!promotion) {
      throw new RpcException(new NotFoundException(`Không tìm thấy khuyến mãi đơn hàng với ID ${id}`));
    }
    ...
    if (updateDto.discountType === DiscountType.FIXED) {
      if (typeof updateDto.discountValue !== 'number' || updateDto.discountValue <= 5000) {
        throw new RpcException(
          new BadRequestException('Với discountType FIXED, discountValue phải là số tiền lớn hơn 5000')
        );
      }
    } else if (updateDto.discountType === DiscountType.PERCENTAGE) {
      if (typeof updateDto.discountValue !== 'number' || updateDto.discountValue < 0 || updateDto.discountValue > 100) {
        throw new RpcException(
          new BadRequestException('Với discountType PERCENTAGE, discountValue phải là số nằm trong khoảng từ 0 đến 100')
        );
      }
    ...
    const updatedPromotion = this.orderPromotionRepository.merge(promotion, updateDto);
