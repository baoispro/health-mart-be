...
export class OrderItemsService implements IOrderItemsService {
  ...
  async findByOrderId(orderId: number): Promise<OrderItem[]> {
    ...
    if (!orderExists) {
      throw new RpcException(
        new NotFoundException(`Order ID ${orderId} không tồn tại`),
      );
    }
  ...
  async getProductPrice(productId: number): Promise<number> {
    try {
      ...
      if (isNaN(price)) {
        throw new RpcException(
          `Invalid price returned for product ${productId}`,
        );
      }
      return price;
    } catch (error) {
      this.logger.error(
        `Failed to get price for product ID ${productId}: ${error.message}`,
      );
  ...
  async createOrderItem(
    ...
    for (const dto of dtosArray) {
      if (typeof dto.product_id !== 'number') {
        throw new RpcException(
          new BadRequestException(
            `product_id phải là số, nhận được: ${dto.product_id}`,
          ),
        );
      }
      ...
      if (typeof dto.quantity !== 'number') {
        throw new RpcException(
          new BadRequestException(
            `quantity phải là số, nhận được: ${dto.quantity}`,
          ),
        );
      }
      ...
      if (typeof dto.order_id !== 'number') {
        throw new RpcException(
          new BadRequestException(
            `order_id phải là số, nhận được: ${dto.order_id}`,
          ),
        );
      }
      ...
      if (!orderExists) {
        throw new RpcException(
          new NotFoundException(
            `Đơn hàng với ID ${dto.order_id} không tồn tại`,
          ),
        );
      }
  ...
  async deleteItemById(orderItemId: number): Promise<{ message: string }> {
    ...
    if (!item) {
      throw new RpcException(
        new NotFoundException(`Không tìm thấy item với ID ${orderItemId}`),
      );
    }
